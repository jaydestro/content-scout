...
// Inputs: a parsed report (output of loadReport) + override thresholds. We
// recompute what would survive under the new filters. The report is already
// post-filter (the agent applied filters at ingest) so this is a "tighten
// further" simulation, not a re-derivation from a raw scan.
export function runReplay({ parsed, sourceFile, overrides = {} }) {
  const minEp = Number.isFinite(overrides.minEp) ? overrides.minEp : null;
  const minScore = Number.isFinite(overrides.minScore) ? overrides.minScore : null;

  const items = Array.isArray(parsed?.items) ? parsed.items : [];
  const kept = [];
  const dropped = [];
  for (const it of items) {
    const ep = Number.isFinite(it.ep) ? it.ep : null;
    let drop = false;
    let reason = '';
    if (minEp != null && (ep == null || ep < minEp)) {
      drop = true;
      reason = `ep ${ep ?? '∅'} < ${minEp}`;
    }
    if (drop) dropped.push({ ...it, reason });
    else kept.push(it);
  }
  const avg = (arr) => {
    const xs = arr.map((x) => x.ep).filter((x) => Number.isFinite(x));
  ...
  const fileName = slug
    ? `${stamp}-${slug}-replay.md`
    : `${stamp}-replay.md`;

  const md = [
    `# Replay — ${sourceFile || 'unknown source'}`,
    '',
    `**Generated:** ${new Date().toISOString()}`,
    `**Source:** ${sourceFile}`,
    `**Overrides:** ${
      minEp != null || minScore != null
        ? `min_ep=${minEp ?? '∅'}, min_score=${minScore ?? '∅'}`
        : 'none (showing baseline only)'
    }`,
    '',
    '## Summary',
    '',
    `Replay applies thresholds to the saved scan without any API calls. Items already filtered out by the original scan cannot be recovered; replay can only tighten further.`,
    '',
    '## Verdict diff',
    '',
    '| Metric | Original | Replay | Δ |',
    '|---|---|---|---|',
    `| Items kept | ${items.length} | ${kept.length} | ${kept.length - items.length} |`,
    `| Items dropped by replay | 0 | ${dropped.length} | +${dropped.length} |`,
    `| Avg engagement potential | ${avg(items)} | ${avg(kept)} | — |`,
    '',
    '## Items dropped by replay',
    '',
    dropped.length
      ? [
          '| # | Title | EP | Reason |',
          ...
          ...dropped.map(
            (it, i) =>
              `| ${i + 1} | ${(it.title || '').replace(/\|/g, '\\|').slice(0, 80)} | ${it.ep ?? '∅'} | ${it.reason} |`
          ),
        ].join('\n')
      : '_No items dropped by these overrides._',
    '',
  ].join('\n');
...
// --- 4. SEO (deterministic, snapshot-only) ------------------------------
