...
// Exported plain-text extractor. The web-ui server uses it to build a body
// excerpt for the optional LLM rewrite pass without re-implementing the strip.
...
// Render the LLM-judgment rewrites for one page, or a fallback note when no
// rewrites were produced. `rw` is the structured object from
// tools/web-ui/lib/seo-rewrite.js (or undefined when no provider ran). Pure:
// the HTTP/agent call lives in the web-ui server, this only formats data.
function seoRewriteSection(rw) {
  if (!rw) {
    return [
      '> _Suggested rewrites were not generated for this audit. Enable **AI rewrites** in the web UI and set `SEO_REWRITE_PROVIDER` in `.env` if you want title, meta description, alternative H1s, opening paragraph, and JSON-LD suggestions inline._',
      '',
    ];
  }
  if (rw.error) {
    return [
      `> _AI rewrites unavailable (${rw.provider || 'provider'}): ${rw.error} The deterministic audit above is complete. Fix the provider and rerun this web UI audit._`,
      '',
    ];
  }
  const lines = ['### Suggested rewrites', ''];
  const by = [rw.provider, rw.model].filter(Boolean).join('/');
  if (by) lines.push(`_Generated by ${by}._`, '');
  if (rw.title) lines.push(`**New \`<title>\`:** "${rw.title}"`, '');
  if (rw.metaDescription) lines.push(`**New \`<meta description>\`:** "${rw.metaDescription}"`, '');
  if (Array.isArray(rw.h1s) && rw.h1s.length) {
    lines.push('**Alternative H1s:**');
    rw.h1s.forEach((h, i) => lines.push(`${i + 1}. ${h}`));
    lines.push('');
  }
  if (rw.openingParagraph) {
    lines.push('**Opening paragraph rewrite:**', `> ${rw.openingParagraph}`, '');
  }
  if (rw.jsonLd) {
    const body = typeof rw.jsonLd === 'string' ? rw.jsonLd : JSON.stringify(rw.jsonLd, null, 2);
    lines.push('**JSON-LD to add:**', '```json', body, '```', '');
  }
  // Model replied but every field was empty. Degrade to a note rather than
  // emitting a bare "### Suggested rewrites" header.
  if (lines.length <= 2) {
    return [
      '> _AI rewrite pass returned no usable content. Adjust the target keywords, audience, or provider model, then rerun this web UI audit._',
      '',
    ];
  }
  return lines;
...
// `rewritesByUrl` optionally maps url -> rewrite object (see seoRewriteSection).
export function runSeoAudit({ pages = [], slug = '', rewritesByUrl = {} }) {
  ...
  const sections = audits.map((a) => {
    ...
    return [
      ...
      '',
      ...seoRewriteSection(rewritesByUrl[a.url]),
    ].join('\n');
  });
