...
function buildPrompt({ productName, summary, author, platform, currentSentiment, userNote }) {
  ...
  return [
    ...
    ...noteBlock,
    ``,
    `Classify the AUTHOR'S STANCE TOWARD ${product}. Use this decision order:`,
    `1. Relevance gate: if the text is hiring, recruiting, job-search,`,
    `   resume/portfolio availability, spam, or only a generic skills/tech`,
    `   stack list, classify unknown. Do not call it neutral just because`,
    `   ${product} is mentioned.`,
    `2. Subject gate: if ${product} is only a passing hashtag, stack item,`,
    `   data-source mention, or one tool in a list, classify unknown unless`,
    `   the author makes a stance toward ${product}.`,
    `3. Stance gate: judge only the author's stance toward ${product}, not`,
    `   their stance toward Azure, AI, a customer project, hiring demand, or`,
    `   a competing product unless they connect it directly to ${product}.`,
    `4. Confidence gate: after choosing the label, set confidence based on`,
    `   how direct the evidence is. Low confidence is a triage signal, not a`,
    `   fallback label.`,
    ``,
    `Use exactly one of:`,
    ...
    `- neutral: on-topic question, informational post, announcement,`,
    `  comparison without a verdict, or retweet without commentary. Neutral`,
    `  requires that ${product} is actually the subject; off-topic/passive`,
    `  mentions are unknown.`,
    ...
    `  trade-off statement, e.g. "solved scale, but RU costs surprised us").`,
    `  Do NOT use mixed for tutorials that include a "common pitfalls" or`,
    `  "gotchas" section — that is teaching, not critique.`,
    `- unknown: not enough context, the body is missing, the post is not`,
    `  really about ${product}, or ${product} is only listed in a hiring`,
    `  role, resume, skills wall, hashtag set, or generic tech stack.`,
    ``,
    ...
    `classify unknown.`,
    ``,
    `Confidence rule: high = explicit evaluative evidence; medium = stance`,
    `inferred from clear context or real trade-off; low = ambiguous, sarcastic,`,
    `translated with uncertainty, missing context, or only tangentially about`,
    `${product}.`,
    ``,
